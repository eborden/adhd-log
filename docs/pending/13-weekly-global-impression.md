...
Named `isWeeklyRecord` (not `isWeekly`) for consistency with the codebase's collection guard
`isDoseChangeList` (the map analog, `parseEntries`, is a `Parsed<T>` parser rather than a
`value is` guard). `isWeeklyRecord` enforces the invariant that the map key
...
The weekly check-in is intentionally **not** wired through the generic daily seams, so no daily
check-in file is touched.
...
- **`app/checkin.tsx` — untouched.** The daily `renderMetric` switch, `Draft` shape,
  ...
  field / switch arm / spread / hydration line is needed. (Adding a `Metric` variant would
  ripple through this generic switch and both sessions' construction; we sidestep that entirely
  by keeping weekly a separate cadence.)
...
- **`app/entry/[date].tsx` — untouched.** This screen is per-day; weekly data is per-week and
  has no per-day detail row. Its rating rows are schema-mapped, and none need editing.
...
daily seams (`checkin.tsx`'s generic `renderMetric` switch, `entry/[date].tsx`'s schema-mapped
rating rows) are explicitly _not_ touched.

## Export / report

`lib/report-html.ts` gains a descriptive **Weekly impression timeline** section and the
`Backup` shape in `lib/backup.ts` grows one field. (The report builder and the `Backup` shape
were split out of `lib/export.ts` in the provider-report overhaul; `lib/export.ts` is now the
RN-ful native-I/O layer — PDF print/share and JSON pick — while `report-html.ts` and
`backup.ts` stay RN-free.) `lib/report-html.ts` imports `palette` from `./tokens` directly and
never `useTheme()` (a React hook that cannot run outside a component and would break the
RN-free Vitest specs).

- **Backup (`lib/backup.ts`):** extend `Backup` to `{ exportedAt; profile; doses; entries; weekly }`.
  `buildBackup` now also takes and serializes the `loadWeekly()` output; `parseBackup` parses
  `weekly` through `parseWeekly` and, for backward compatibility with pre-feature backups, treats
  a **missing** `weekly` key as `{}` (present-but-malformed still fails the parse). `restoreBackup`
  (in `lib/storage.ts`) writes it via `saveWeekly`, extending its existing `Promise.all` over
  profile/doses/entries; `importJsonBackup` (in `lib/export.ts`) just parses and returns the
  `Backup`. Any future doc adding a top-level `Backup` field should reuse this same
...
- **Two pure, tested confounder helpers** (RN-free, in coverage scope), so the provider sees
  ...
  `doseTaken` is already collected on `MorningCheckin` and now appears per-day in the
  schema-driven daily-log table, but it is not _aggregated per week_ beside the weekly
  impression, so without this helper the week's adherence would be invisible next to the exact
  row meant to summarize the week — the single biggest confounder for a titration read.
  `weeklyAdherence` surfaces it as "N/7 logged, M taken." (IsoDate string comparison in
  `doseChangeInWeek` is safe — ISO dates sort lexicographically.)

- **Report HTML (`lib/report-html.ts`):** add `buildWeeklyTimelineHtml(weekly, entries, doses)` producing a table
  ...
  `buildReportHtml` (current signature `(profile, doses, entries, rangeStart, rangeEnd, options)`)
  already receives the full `entries` map, so it only gains a `weekly` argument and inserts this
  section after the dose-change list and before the daily-log table. (If another doc in this set
  also extends `buildReportHtml`, land the signature change in one pass.)
...
- Averaging machinery (`averageOf` / `ratingAccessor` in `lib/metrics.ts`) is untouched — weekly
  data is categorical and never averaged.
...
(`lib/{types,schema,storage,backup,report-html,metrics}.ts` — `export.ts` is now the RN-ful
native-I/O layer and is not unit-tested). Tests import `{ describe, it, expect }` from
...
`lib/__tests__/report-html.test.ts` (extend):
...
- **`impressionGlyph`** exhaustiveness is enforced by the compiler (`assertNever`); a test
  ...
  `lib/__tests__/backup.test.ts` (extend):
...
- **RN-free `lib/report-html.ts`:** chip colors come from `palette` in `./tokens` directly, never
...
- **`theme.*` colors in RN-free `lib/report-html.ts` (must-fix):** applied. Chip colors read from `palette` in `./tokens` directly (mirroring `buildReportHtml`); no `useTheme()`.

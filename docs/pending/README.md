...
- **User-value / features (06–51)** — 06–16 distilled from the user-value analysis (2026-07-18);
  17–20 distilled from the titration-log research (2026-07-21, `docs/research/titration-log-examples.md`);
  21 a clinical-lens alternative to #13; 22–51 six rounds of an innovation batch (2026-07-23)
...
| 47  | [Symptom photo diary](47-symptom-photo-diary.md)                             | P3       | Decision doc: optional local-only photo per side-effect-day, reframed from an initial provider-facing pitch to an honest patient-side recall aid; leans toward not building given a new binary-asset architecture and an OS-auto-backup exposure the clinical panel required naming explicitly |
| 48  | [Quarantine / data-health viewer](48-quarantine-viewer.md)                   | P2       | Surfaces doc 03's landed-but-invisible `quarantineEntries` safety net — list/view/delete quarantined blobs from Settings, empty-by-default, with a structural key-shape guard on the delete path                                                                                               |
| 49  | [Accessibility pass](49-accessibility-pass.md)                               | **P1**   | Dynamic Type / VoiceOver-TalkBack / tap-target audit across every screen — verified against real component code which needs enriching (partial support already present) vs. a genuine gap (no accessibility props at all)                                                                      |
| 50  | [Metric-enablement change log](50-metric-enablement-log.md)                  | P2       | Append-only log of `enabledEveningMetrics` toggles powering symmetric "tracked since"/"tracking stopped" captions — closes a trend-data-honesty gap doc 09 didn't cover for tracking-scope changes rather than logging gaps                                                                    |
| 51  | [Multi-day side-effect backfill](51-side-effect-backfill.md)                 | P2       | Applies one side effect + severity across a date range into existing evening check-ins only, marked `origin: 'backfilled'`; reworked after the strict-TypeScript lens rejected the original toggle-then-set merge logic, which deleted an already-present effect instead of confirming it      |

### How the 06–51 docs were produced
...
`docs/research/titration-log-examples.md`; 22–26, 27–31, 32–36, 37–41, and 47–51 followed it
again on 2026-07-23 as six rounds of an innovation batch, each with independently-spawned
reviewers):
...
Round 6 (47–51) added a third **reject**, again from the strict-TypeScript lens, corroborated
independently by the data-model/scope lens: doc 51's original design looped the single-day
`withSideEffectToggled`/`withSideEffectSeverity` merge across a date range, which actually
**deletes** an already-present effect instead of confirming it — a genuine data-loss bug, not a
copy issue — caught by reading the real `lib/schema.ts` functions rather than trusting the doc's
own description of them. It was reworked around a new, additive `withSideEffectAtSeverity`
export with a direct regression test. Round 6 also produced this batch's most privacy-sensitive
finding: doc 47's photo-diary design surfaced a real OS-level auto-backup exposure (iCloud/Google
auto-backup capturing the app's sandboxed document directory by default) that this app's own
JSON-export exclusion does not address, alongside a framing contradiction (claiming provider
value for data that, by the doc's own design, never reaches the report) — both required a
substantial reframing of the doc's Problem and Recommendation sections, which now lean toward not
building. Doc 50's implementation of its own must-fix additionally surfaced an inverted return
condition in the originally-drafted `trackedSince` helper (the enabled/disabled branches were
backwards relative to the doc's own stated behavior and test plan), fixed as part of applying the
clinical lens's disable-direction must-fix.
